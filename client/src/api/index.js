...
export const publicApi = {
  getEventType: (slug) => api.get(`/public/event-types/${slug}`).then(r => r.data),
  getSlots: (slug, date) => api.get(`/public/slots/${slug}`, { params: { date } }).then(r => r.data),
  getAvailableDates: (slug, month) =>
    api.get(`/public/available-dates/${slug}`, { params: { month } }).then(r => r.data),
  book: (data) => api.post('/public/book', data).then(r => r.data),
  getBooking: (id) => api.get(`/public/bookings/${id}`).then(r => r.data),
};
